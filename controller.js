...
exports.updateMosaic = {
    ...
    handler: function (request, reply) {
        ...
        if (data.file) {
            ...
            data.file.on('end', function () {
                console.log(name);
                var cmd = 'curl -v -u admin:geoserver -XPOST -H "Content-type: text/plain" -d "/home/ubuntu/tifs/' + name + '" http://localhost/geoserver/rest/workspaces/mosaic/coveragestores/' + store + '/external.imagemosaic';
                console.log(cmd);
                exec(cmd, { maxBuffer: 314572800 }, function (error, stderr, stdout) {
                    if (error) {
                        reply(boom.expectationFailed(error, stderr));
